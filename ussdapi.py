...
def ussd_handler():
    ...
    ussd_response = USSDResponse(session_id=ussd_request.get('sessionID'),
    ...
    if ussd_request.get('newSession') and ussd_request.get('userData') == '*928*92#':
        ussd_response.message = "Welcome to Arkesel Bank\n1. Check Balance\n2. Buy Bundle"
        ussd_response.continueSession = True
    elif not ussd_request.get('newSession') and ussd_request.get('userData') == '1':
        ussd_response.message = "Your account balance is GHc 2.00"
        ussd_response.continueSession = False
    elif not ussd_request.get('newSession') and ussd_request.get('userData') == '2':
        ussd_response.message = "No packages available for subscription"
        ussd_response.continueSession = False
    else:
